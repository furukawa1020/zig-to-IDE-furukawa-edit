...
fn generateSite(allocator: std.mem.Allocator, out_dir: []const u8) !void {
    ...
    try copyLicense(allocator, out_dir);
...
fn copyLicense(allocator: std.mem.Allocator, out_dir: []const u8) !void {
    const bytes = try std.Io.Dir.cwd().readFileAlloc(std.Options.debug_io, "LICENSE", allocator, .limited(128 * 1024));
    defer allocator.free(bytes);
    try writeAssetBytes(allocator, out_dir, "LICENSE", bytes);
}
...
fn writeHtml(allocator: std.mem.Allocator, out_dir: []const u8, file_name: []const u8, downloads: DownloadSet) !void {
    ...
    try writer.print(
        ...
        \\  <footer>
        \\    <span>ZIDE</span>
        \\    <span>Generated by Zig. No Node, no frontend build chain. <a href="LICENSE">MIT License</a>.</span>
        \\  </footer>
        \\</body>
        \\</html>
        \\
